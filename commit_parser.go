...
func newCommitParser(client gitcmd.Client, config *chglog.Config) *commitParser {
	opts := config.Options
...
func (p *commitParser) Parse(rev string) ([]*chglog.Commit, error) {
	...
	lines = lines[1:]
	commits := make([]*chglog.Commit, len(lines))
...
func (p *commitParser) parseCommit(input string) *chglog.Commit {
	commit := &chglog.Commit{}
...
func (p *commitParser) parseHash(input string) *chglog.Hash {
...
func (p *commitParser) parseAuthor(input string) *chglog.Author {
	arr := strings.Split(input, "\t")
...
func (p *commitParser) parseCommitter(input string) *chglog.Committer {
	author := p.parseAuthor(input)

	return &chglog.Committer{
		Name:  author.Name,
		Email: author.Email,
		Date:  author.Date,
	}
...
func (p *commitParser) processHeader(commit *chglog.Commit, input string) {
...
func (p *commitParser) processBody(commit *chglog.Commit, input string) {
	...
	for _, line := range lines {
		...
		if len(res) > 0 {
			inNote = true
			for _, r := range res {
				commit.Notes = append(commit.Notes, &chglog.Note{
					Title: r[1],
					Body:  r[2],
				})
			}
...
func (*commitParser) trimSpaceInNotes(commit *chglog.Commit) {
	for _, note := range commit.Notes {
		note.Body = strings.TrimSpace(note.Body)
	}
}

func (p *commitParser) parseRefs(input string) []*chglog.Ref {
	refs := []*chglog.Ref{}
...
func (p *commitParser) uniqRefs(refs []*chglog.Ref) []*chglog.Ref {
	arr := []*chglog.Ref{}
